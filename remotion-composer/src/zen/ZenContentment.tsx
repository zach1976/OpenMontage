...
// ---------------------------------------------------------------------------
const handle = delayRender("content-fonts", { timeoutInMilliseconds: 240000 });
Promise.all([
...
// audio durations (s)
// karaoke: per-word start times inside v4_th.mp3 (phrase-natural read)
const TOKEN_STARTS = [0.0, 0.533, 1.29, 1.534, 1.696];
const V4_TH_TOTAL = 1.94;
const VO_OFFSET = 0.5; // v4_th starts 0.5s into the sentence/recap card

const DUR: Record<string, number> = {
  v4_hook: 3.96,
  v4_th: 1.94, v4_zh: 3.528,
  zh_cta: 3.672,
  v4w1_th: 1.776, v4w2_th: 1.896, v4w3_th: 1.776, v4w4_th: 1.776,
  v4w1_zh: 1.92, v4w2_zh: 1.752, v4w3_zh: 1.824, v4w4_zh: 1.776,
};
...
const WORDS = [
  { th: "ความสุข", roman: "khwaam-sùk", zh: "幸福" },
  { th: "ไม่ใช่", roman: "mâi-châi", zh: "不是" },
  { th: "มี", roman: "mii", zh: "拥有 · 有" },
  { th: "มาก", roman: "mâak", zh: "多" },
];
...
cue(0.6, DUR.v4_hook, "v4_hook");

let cur = HOOK.end;
// full sentence (Thai then Chinese)
const sTh = cur + 0.5;
const sZh = sTh + DUR.v4_th + 0.4;
const SENT: Block = { start: cur, end: sZh + DUR.v4_zh + 0.8 };
cue(sTh, DUR.v4_th, "v4_th");
cue(sZh, DUR.v4_zh, "v4_zh");
cur = SENT.end;
...
WORDS.forEach((_, i) => {
  const start = cur;
  const t = start + 0.35;
  const z = t + DUR[`v4w${i + 1}_th`] + 0.25;
  const end = z + DUR[`v4w${i + 1}_zh`] + 0.55;
  cue(t, DUR[`v4w${i + 1}_th`], `v4w${i + 1}_th`);
  cue(z, DUR[`v4w${i + 1}_zh`], `v4w${i + 1}_zh`);
...
const rTh = cur + 0.5;
const RECAP: Block = { start: cur, end: rTh + DUR.v4_th + 0.9 };
cue(rTh, DUR.v4_th, "v4_th");
cur = RECAP.end;
...
const SentenceCard: React.FC<{ dur: number; label?: string }> = ({ dur, label }) => {
  ...
  let active = -1;
  if (elapsed >= 0 && elapsed <= V4_TH_TOTAL + 0.25) {
...
export const ZenContentment: React.FC<{ variant: ZenVariant }> = ({ variant }) => {
  return (
    <AbsoluteFill style={{ backgroundColor: "#0a1420" }}>
      <AbsoluteFill>
        <VideoLayer src="zen/scenes/scene_sakura.mp4" loopFrames={818} />
        <Sequence from={s(18)} name="bg-2">
          <FadeInBg fade={40}><VideoLayer src="zen/scenes/scene_flowers.mp4" loopFrames={356} /></FadeInBg>
        </Sequence>
        <Scrim />
      </AbsoluteFill>
      <SeriesTag />
...
export const CONTENT_TOTAL_FRAMES = s(TOTAL) + 4;
