...
// ---------------------------------------------------------------------------
const handle = delayRender("calm-fonts", { timeoutInMilliseconds: 240000 });
Promise.all([
...
// audio durations (s)
// karaoke: per-word start times inside v3_th.mp3 (phrase-natural read)
const TOKEN_STARTS = [0.0, 0.203, 0.506, 1.414, 1.871];
const V3_TH_TOTAL = 2.328;
const VO_OFFSET = 0.5; // v3_th starts 0.5s into the sentence/recap card

const DUR: Record<string, number> = {
  v3_hook: 4.2,
  v3_th: 2.328, v3_zh: 3.912,
  zh_cta: 3.672,
  v3w1_th: 1.776, v3w2_th: 1.776, v3w3_th: 1.848, v3w4_th: 1.896,
  v3w1_zh: 1.776, v3w2_zh: 1.824, v3w3_zh: 1.776, v3w4_zh: 1.776,
};
...
const WORDS = [
  { th: "ใจ", roman: "jai", zh: "心" },
  { th: "สงบ", roman: "sà-ngòp", zh: "平静" },
  { th: "ไม่มี", roman: "mâi-mii", zh: "没有" },
  { th: "ปัญหา", roman: "pan-hǎa", zh: "问题" },
];
...
cue(0.6, DUR.v3_hook, "v3_hook");

let cur = HOOK.end;
// full sentence (Thai then Chinese)
const sTh = cur + 0.5;
const sZh = sTh + DUR.v3_th + 0.4;
const SENT: Block = { start: cur, end: sZh + DUR.v3_zh + 0.8 };
cue(sTh, DUR.v3_th, "v3_th");
cue(sZh, DUR.v3_zh, "v3_zh");
cur = SENT.end;
...
WORDS.forEach((_, i) => {
  const start = cur;
  const t = start + 0.35;
  const z = t + DUR[`v3w${i + 1}_th`] + 0.25;
  const end = z + DUR[`v3w${i + 1}_zh`] + 0.55;
  cue(t, DUR[`v3w${i + 1}_th`], `v3w${i + 1}_th`);
  cue(z, DUR[`v3w${i + 1}_zh`], `v3w${i + 1}_zh`);
...
const rTh = cur + 0.5;
const RECAP: Block = { start: cur, end: rTh + DUR.v3_th + 0.9 };
cue(rTh, DUR.v3_th, "v3_th");
cur = RECAP.end;
...
const SentenceCard: React.FC<{ dur: number; label?: string }> = ({ dur, label }) => {
  ...
  let active = -1;
  if (elapsed >= 0 && elapsed <= V3_TH_TOTAL + 0.25) {
...
export const ZenCalm: React.FC<{ variant: ZenVariant }> = ({ variant }) => {
  return (
    <AbsoluteFill style={{ backgroundColor: "#0a1420" }}>
      <AbsoluteFill>
        <VideoLayer src="zen/scenes/scene_lake1.mp4" loopFrames={518} />
        <Sequence from={s(20)} name="bg-2">
          <FadeInBg fade={40}><VideoLayer src="zen/scenes/scene_lake2.mp4" loopFrames={462} /></FadeInBg>
        </Sequence>
        <Scrim />
      </AbsoluteFill>
      <SeriesTag />
...
export const CALM_TOTAL_FRAMES = s(TOTAL) + 4;
