...
// ---------------------------------------------------------------------------
const handle = delayRender("lettinggo-fonts", { timeoutInMilliseconds: 240000 });
Promise.all([
...
// audio durations (s)
// karaoke: per-word start times inside v2_th.mp3 (phrase-natural read)
const TOKEN_STARTS = [0.0, 0.217, 0.796, 1.53, 1.719, 1.908];
const V2_TH_TOTAL = 2.412;
const VO_OFFSET = 0.5; // v2_th starts 0.5s into the sentence/recap card

const DUR: Record<string, number> = {
  v2_hook: 3.96,
  v2_th: 2.412, v2_zh: 3.792,
  zh_cta: 3.672,
  v2w1_th: 1.848, v2w2_th: 1.896, v2w3_th: 1.68, v2w4_th: 1.8,
  v2w1_zh: 1.848, v2w2_zh: 1.776, v2w3_zh: 1.776, v2w4_zh: 1.8,
};
...
const WORDS = [
  { th: "ปล่อยวาง", roman: "plòi-waang", zh: "放下" },
  { th: "ไม่ใช่", roman: "mâi-châi", zh: "不是" },
  { th: "ลืม", roman: "lʉʉm", zh: "忘记" },
  { th: "ทุกอย่าง", roman: "túk-yàang", zh: "一切 · 每件事" },
];
...
cue(0.6, DUR.v2_hook, "v2_hook");

let cur = HOOK.end;
// full sentence (Thai then Chinese)
const sTh = cur + 0.5;
const sZh = sTh + DUR.v2_th + 0.4;
const SENT: Block = { start: cur, end: sZh + DUR.v2_zh + 0.8 };
cue(sTh, DUR.v2_th, "v2_th");
cue(sZh, DUR.v2_zh, "v2_zh");
cur = SENT.end;
...
WORDS.forEach((_, i) => {
  const start = cur;
  const t = start + 0.35;
  const z = t + DUR[`v2w${i + 1}_th`] + 0.25;
  const end = z + DUR[`v2w${i + 1}_zh`] + 0.55;
  cue(t, DUR[`v2w${i + 1}_th`], `v2w${i + 1}_th`);
  cue(z, DUR[`v2w${i + 1}_zh`], `v2w${i + 1}_zh`);
...
const rTh = cur + 0.5;
const RECAP: Block = { start: cur, end: rTh + DUR.v2_th + 0.9 };
cue(rTh, DUR.v2_th, "v2_th");
cur = RECAP.end;
...
const SentenceCard: React.FC<{ dur: number; label?: string }> = ({ dur, label }) => {
  ...
  let active = -1;
  if (elapsed >= 0 && elapsed <= V2_TH_TOTAL + 0.25) {
...
export const ZenLettingGo: React.FC<{ variant: ZenVariant }> = ({ variant }) => {
  return (
    <AbsoluteFill style={{ backgroundColor: "#0a1420" }}>
      <AbsoluteFill>
        <VideoLayer src="zen/scenes/scene_lantern1.mp4" loopFrames={309} />
        <Sequence from={s(20)} name="bg-2">
          <FadeInBg fade={40}><VideoLayer src="zen/scenes/scene_lantern2.mp4" loopFrames={659} /></FadeInBg>
        </Sequence>
        <Scrim />
      </AbsoluteFill>
      <SeriesTag />
...
export const LETTINGGO_TOTAL_FRAMES = s(TOTAL) + 4;
