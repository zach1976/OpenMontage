...
// ---------------------------------------------------------------------------
const handle = delayRender("zen-fonts", { timeoutInMilliseconds: 240000 });
Promise.all([
...
// audio durations (s)
// karaoke: per-word start times inside th_r1.mp3 (phrase-natural read)
const TOKEN_STARTS = [0.0, 0.335, 0.603, 0.87, 1.071, 1.572, 1.871];
const TH_R1_TOTAL = 2.47;
const VO_OFFSET = 0.5; // th_r1 starts 0.5s into the sentence/recap card

const DUR: Record<string, number> = {
  zh_hook: 4.344,
  th_r1: 2.47, zh_r1: 4.296,
  zh_cta: 3.672,
  bw1_th: 1.896, bw2_th: 1.776, bw3_th: 1.584, bw4_th: 1.776, bw5_th: 1.776, bw6_th: 1.776, bw7_th: 2.064,
  bw1_zh: 1.776, bw2_zh: 1.8, bw3_zh: 1.896, bw4_zh: 2.4, bw5_zh: 2.04, bw6_zh: 1.776, bw7_zh: 1.8,
};
...
const SENTENCE = {
  th: [
    ["ไม่มี", "อะไร", "อยู่", "กับ", "เรา"],
    ["ได้", "ตลอดไป"],
  ],
  roman: "mâi mii à-rai yùu gàp rao — dâai talɔ̀ɔt-pai",
  zh: "没有什么，能永远陪着我们",
};
const WORDS = [
  { th: "ไม่มี", roman: "mâi mii", zh: "没有" },
  { th: "อะไร", roman: "à-rai", zh: "什么" },
  { th: "อยู่", roman: "yùu", zh: "停留 · 在" },
  { th: "กับ", roman: "gàp", zh: "和 · 跟" },
  { th: "เรา", roman: "rao", zh: "我们" },
  { th: "ได้", roman: "dâai", zh: "能够" },
  { th: "ตลอดไป", roman: "talɔ̀ɔt-pai", zh: "永远" },
];
...
cue(0.6, DUR.zh_hook, "zh_hook");

let cur = HOOK.end;
// full sentence (Thai then Chinese)
const sTh = cur + 0.5;
const sZh = sTh + DUR.th_r1 + 0.4;
const SENT: Block = { start: cur, end: sZh + DUR.zh_r1 + 0.8 };
cue(sTh, DUR.th_r1, "th_r1");
cue(sZh, DUR.zh_r1, "zh_r1");
cur = SENT.end;
...
WORDS.forEach((_, i) => {
  const start = cur;
  const t = start + 0.35;
  const z = t + DUR[`bw${i + 1}_th`] + 0.25;
  const end = z + DUR[`bw${i + 1}_zh`] + 0.55;
  cue(t, DUR[`bw${i + 1}_th`], `bw${i + 1}_th`);
  cue(z, DUR[`bw${i + 1}_zh`], `bw${i + 1}_zh`);
...
const rTh = cur + 0.5;
const RECAP: Block = { start: cur, end: rTh + DUR.th_r1 + 0.9 };
cue(rTh, DUR.th_r1, "th_r1");
cur = RECAP.end;
...
const SentenceCard: React.FC<{ dur: number; label?: string }> = ({ dur, label }) => {
  ...
  let active = -1;
  if (elapsed >= 0 && elapsed <= TH_R1_TOTAL + 0.25) {
...
export const ZenImpermanenceFull: React.FC<{ variant: ZenVariant }> = ({ variant }) => {
  return (
    <AbsoluteFill style={{ backgroundColor: "#0a1420" }}>
      <AbsoluteFill>
        <VideoLayer src="zen/scenes/scene_mountain.mp4" loopFrames={359} />
        <Sequence from={s(34)} name="bg-city">
          <FadeInBg fade={40}><VideoLayer src="zen/scenes/scene_city.mp4" loopFrames={529} /></FadeInBg>
        </Sequence>
        <Scrim />
      </AbsoluteFill>
      <SeriesTag />
...
export const ZEN_TOTAL_FRAMES = s(TOTAL) + 4;
